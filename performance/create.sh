...
curl -u "$credentials" "$host/api/v1/namespaces/_/actions/$action" -XPUT -d '{"namespace":"_","name":"test","exec":{"kind":"nodejs:default","code":"function main(){var stop = new Date().getTime();while(new Date().getTime() < stop + 10000) {;}return {};}"}}' -H "Content-Type: application/json"
